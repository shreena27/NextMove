...
// Fills route through TOKENS, not re-typed hex literals — Gems is exactly
// the decorative element PRD §16's reserved-colour rule is about, and a
// literal hex here would be invisible to the ΔE floor test that scans
// NON_STATUS_TOKENS (tokens.ts). No `Record<string, ReactElement>`
// annotation: leaving SHAPES's type inferred keeps `keyof typeof SHAPES`
// (used by LAYOUTS below) a real, checked union instead of collapsing to
// plain `string`.
const SHAPES = {
  diamondGreen: (
    <svg width="16" height="16" viewBox="0 0 16 16">
      <rect x="3" y="3" width="10" height="10" rx="2" transform="rotate(45 8 8)" fill={TOKENS['sq-green']} />
    </svg>
  ),
  diamondPink: (
    <svg width="13" height="13" viewBox="0 0 16 16">
      <rect x="3" y="3" width="10" height="10" rx="2" transform="rotate(45 8 8)" fill={TOKENS['sq-pink']} />
    </svg>
  ),
  dotButter: (
    <svg width="11" height="11" viewBox="0 0 12 12">
      <circle cx="6" cy="6" r="5" fill={TOKENS.butter} stroke={TOKENS.ink} strokeWidth={1} />
    </svg>
  ),
  ringBlue: (
    <svg width="14" height="14" viewBox="0 0 16 16">
      <circle cx="8" cy="8" r="5.5" fill="none" stroke={TOKENS['sq-blue']} strokeWidth={2.5} />
    </svg>
  ),
  star: (
